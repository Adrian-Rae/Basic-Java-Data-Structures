...
class BTreeNode<T extends Comparable<T>> {
	...
	private void printKeys()
	{
		...
		System.out.print("[");
    		...
    		{
        		j=i;
				System.out.print(" " + (this.references[i]==null?"() ":"("+references[i]+") ") + this.keys[i]);
    		}
			System.out.print(" " + (this.references[j+1]==null?"() ":"("+references[j+1]+") "));
 		System.out.print("]");
		//*/
	}
    ...
    {  
		if(this != null){
			int i =0;
			while(i<keyTally && keys[i] !=null && keys[i].compareTo(key) < 0){i++;}
			//System.out.println("Following link "+i);
			if(i<2*m-1 && keys[i] == key){ return this;}
	...
	public void insertNonFullRoot(BTreeNode<T> x, T key){
		...
		if(x.leaf)
		{
			...
			while(i >= 1 && x.keys[i-1] != null && x.keys[i-1].compareTo(key)>0)
			{
		...
		{
			int j = 0;
			while(j < x.keyTally  && x.keys[j]!=null && x.keys[j].compareTo(key) < 0){j++;}
	...
	private boolean insertingIntoFullLeafNode(T key){
		...
		int i = 0;
		while(i<this.keyTally && keys[i]!=null && keys[i].compareTo(key)<0){i++;}
